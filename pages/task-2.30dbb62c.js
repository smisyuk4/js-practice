...
const onTargetMoveBall = (event)=>{
    ...
    if (posBall.top < 0) posBall.top = 0;
    //заборона перетину правої межі
    if (posBall.left + ball.clientWidth > field.clientWidth) posBall.left = field.clientWidth - ball.clientWidth;
    //заборона перетину нижньої межі
    if (posBall.top + ball.clientHeight > field.clientHeight) posBall.top = field.clientHeight - ball.clientHeight;
    //заборона перетину лівої межі
    if (posBall.left < 0) posBall.left = 0;
